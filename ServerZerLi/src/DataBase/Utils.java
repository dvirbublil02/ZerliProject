...
public class Utils {
	/**This method will add the user that connected to the server ,
	 *  into his window on the server (by calling ServerScreenController.setObser
	* and updating the obj.response if its success or not.
	 */
	public static void updateConnectionWindow(TransmissionPack obj) {
		if (obj.getInformation() != null) {
			...
			ServerScreenController.SetObser(obj);
		} else
