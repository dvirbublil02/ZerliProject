...
public class OrderReportsController implements Initializable {
	...
	private void insertTheOrdersReportDetails(ObservableList<PieChart.Data> pieChartData,
		...
		reportTitle.setText("Zerli "+branchInfo.get(1)+"(ID-"+branchInfo.get(0)+")"+" - " +branchInfo.get(2)+"-th orders based on products");
		// LineChart
		...
		amount=insertIntoChart(bouquet, single, series1, series2); //insert the products their information into the chart
		...
		for(int i=1;i<reportOnList.size();i++) {
			...
			if(productInfo.get(0).equals("product")) {
				...
				if(flag==false) {
				 pieChartData = FXCollections.observableArrayList(new PieChart.Data(name.toString(), ((Integer.parseInt(productInfo.get(3))/amount.get(0)))));
				
				}
				else {
					pieChartData.add(new PieChart.Data(name.toString(), ((Integer.parseInt(productInfo.get(3))/amount.get(0)))));
				}
				 flag=true;
			}
			if(productInfo.get(0).equals("item")) {
				...
				name.append(productInfo.get(2));
				if(flag2==false) {
					 pieChartData2 = FXCollections.observableArrayList(new PieChart.Data(name.toString(), ((Integer.parseInt(productInfo.get(3))/amount.get(0)))));
					 
					}
					else {
						pieChartData2.add(new PieChart.Data(name.toString(), ((Integer.parseInt(productInfo.get(3))/amount.get(0)))));
					}
				flag2=true;
			}
		}
...
//////////////////////---PieChart MouseEvent(regular + Custom)--//////////////////////////////////////////
	...
	private void insertTheWorstAndBestSell() {
		...
		for(int i=1;i<reportOnList.size();i++) {
			...
			if(Integer.parseInt(productInfo.get(3))>bestSellerAmount) {
				...
				if(Integer.parseInt(productInfo.get(3))<worstSellerAmount) {
	...
	/**
	 *  this method insert into the diagram the products and their value according to the report , that we got from the DB
	 * @param bouquet
	 * @param single
	 * @param series1
	 * @param series2
